...
class RNV_Source(menus.ListPageSource):
    async def format_page(self, menu, entries):
        ...
        if maximum > 1:
            footer = f'Page {menu.current_page + 1}/{maximum} ({len(self.entries)} members)'
...
class NewSimpage(BaseNewButton):
    ...
    def __init__(self, entries, *, ctx: commands.Context, per_page: int = 12 , member_list):
        super().__init__(RNV_Source(entries, per_page=per_page), ctx=ctx)
